...
public class MibRecord implements Serializable {
	 ...
	 * recordType:
	 * 	= 1  =>  it is ITSELF a table (i.e. its syntax is a Sequence)
	 * 	= 2  =>  it is a entry (elements under "Entry" are actual elements
	 * 	= 3  =>  it is an element. Index values are appended to it instead of .0
	...
	public int getSyntaxID() {
		...
		if (strType.indexOf("INTEGER") != -1)		return VALUE_TYPE_INTEGER32;
		else if (strType.indexOf("COUNTER") != -1)	return VALUE_TYPE_UNSIGNED_INTEGER32;
		else if (strType.indexOf("STRING") != -1)	return VALUE_TYPE_OCTET_STRING;
		else if (strType.indexOf("OID")!= -1)		return VALUE_TYPE_OID;
		else if (strType.indexOf("TIMETICK") != -1)	return VALUE_TYPE_TIMETICKS;
		else if (strType.indexOf("IPADDRESS") != -1)return VALUE_TYPE_IP_ADDRESS;
		else if (strType.indexOf("NULL") != -1)		return VALUE_TYPE_NULL;
		return VALUE_TYPE_NONE;
	}

	public String getSyntaxIDString() {
		switch (getSyntaxID()) {
		case VALUE_TYPE_INTEGER32:					return "Integer";
		case VALUE_TYPE_UNSIGNED_INTEGER32:			return "Unsigned Integer";
		case VALUE_TYPE_OCTET_STRING:				return "Octet String";
		case VALUE_TYPE_OID:						return "OID";
		case VALUE_TYPE_TIMETICKS:					return "Time Ticks";
		case VALUE_TYPE_IP_ADDRESS:					return "IP Address";
	...
	public boolean equals(Object other) {
		...
		MibRecord mib = (MibRecord) other;
	    return name == mib.name || (name != null && name.equals(mib.name));
	}
